...
3、使用 curl 访问 URL 时，报错如下：
```markdown
{"code":14,"message":"connection error: desc = \"transport: authentication handshake failed: x509: certificate relies on legacy Common Name field, use SANs or temporarily enable Common Name matching with GODEBUG=x509ignoreCN=0\"","details":[]}%         
```

grpc 服务端启动时，增加环境变量 `GODEBUG=x509ignoreCN=0`